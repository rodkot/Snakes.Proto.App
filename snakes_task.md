...
В этом сообщении указывается имя игры, к которой хотим присоединиться (важно для "сервера", где могут одновременно идти несколько игр).
...
Но есть следующие отличия:
 - "сервер" не рассылает сообщения AnnouncementMsg multicast-ом;
 - "сервер" может вести сразу несколько игр параллельно (не более 1000), игры отличаются по имени;
 - "сервер" не имеет никакого интерфейса, кроме описанного протокола взаимодействия;
 - можно попросить "сервер" присоединиться к вашей игре путём отправки ему сообщения AnnouncementMsg;
 - "сервер" автоматически управляет своей змейкой в каждой игре.

Можно подключиться к любой уже идущей игре, или указать новое имя, тогда сервер должен создать новую игру с таким именем.
...
В случае, когда узел имеет роль MASTER, нужно предусмотреть возможность отправки "серверу" запроса на подключение к нашей игре (AnnouncementMsg).
